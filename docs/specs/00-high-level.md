...
Four concrete consequences of this vision:
...
3. **Providers are plug-ins, not config blocks.** A provider is a code module in `src/providers/<name>/` that owns its own auth flow (API key, device flow, future OAuth), its own model catalog (static or dynamically discovered), and the BYOK config the Copilot SDK consumes. Adding a new provider = writing a TypeScript module + a preset file. The gateway and the LLM backend talk to providers exclusively through the `Provider` interface in `08-providers.md`.
4. **The agent loop is upstream.** We don't reimplement tool-calling or message history. The boundary between "what the Copilot SDK does" and "what we do" is the central architectural fact and is loud in every boundary spec.
...
| 08 | [`providers.md`](08-providers.md) | The `Provider` interface, `BaseProvider`, `PresetConfig` shape, `src/providers/` directory layout, CLI `provider` subcommands, LLM-backend BYOK construction seam | Channel I/O, agent prompts, session state |
...
- **Plugin code lives in the source tree, not the config dir.** Channel adapters (`src/channels/<name>/`) and providers (`src/providers/<name>/`) are TypeScript modules committed to the repo. Per-deployment state (auth tokens, chosen model) lives in `<configDir>/presets/<name>.yaml` and is created by the CLI. New channels = new code module. New providers = new code module + a preset file. (See `08-providers.md` for the `Provider` interface; `01-channel-adapter.md` for the `ChannelAdapter` interface.)
- **Compile-time plugin loading (v1).** The gateway has hardcoded `import` statements for each enabled channel adapter (in `src/channels/registry.ts`) and each available provider (in `src/providers/registry.ts`), registered at startup via a `registerChannel(adapter)` and `registerProvider(provider)` API respectively. The APIs are clean enough that a future runtime loader could be added without changing the gateway. *YAGNI for runtime discovery in v1.*
...
| 8 | Dynamic model discovery | **In v1** — every provider whose `capabilities.dynamicModels` is true implements `discoverModels()`; the gateway calls it at startup and caches per the catalog's `ttlMs` (default 1 h). | `08-providers.md` |
| 9 | CLI subcommand scope | **`provider`, `agent`, `config`, `channels`** all in v1, shipped in the same `copilot-agent` binary as the bot. CLI and WebUI share the same Zod-validated paths so the two surfaces can't drift. | `08-providers.md`, `03`, `07`, `01` |
| 10 | Plugin code location | **`src/providers/<name>/`** for provider code, **`src/channels/<name>/`** for channel adapter code, both with `_shared/` for common utilities. Per-deployment state lives in `<configDir>/presets/<name>.yaml`. | `08-providers.md`, `01-channel-adapter.md` |
| 11 | OAuth flow (`capabilities.oauthFlow`) | **Deferred to v2** — `AuthConfig.oauth` and `ProviderCapabilities.oauthFlow` are reserved in the type system; v1 supports `api_key` and `device_flow` only. | `08-providers.md` |
